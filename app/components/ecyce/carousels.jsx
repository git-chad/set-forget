...
const Carousels = ({ type }) => {
  ...
  return (
    <div className="flex items-center h-52 bg-sf-beige text-sf-dblue hover:text-sf-green transition-colors duration-700 text-[5rem] bold-neue overflow-hidden cursor-default">
        <div ref={scope} className="flex items-center w-[2716px] gap-[60px]">
            {talk.map((word, index) => (
            <span key={index} className="inline-block flex-shrink-0">
                {word}
